...
writing it back, so a snapshot can never resurrect a false claim. Also confirms AgDR-0003's
rule: a snapshot can only be restored into the SAME environment it came from."""
import sys
import tempfile
import threading
import unittest
...
class EnvironmentSnapshotRestoreTests(unittest.TestCase):
    ...
    def test_cannot_restore_a_snapshot_into_a_different_environment(self):
        # AgDR-0003: a snapshot's resources are owned by the SNAPSHOT's environment. Restoring
        # into a different environment would claim them for it, so it must be rejected, not
        # silently allowed.
        self._create_and_provision("app-a")
        snap = mcp_server.snapshot_environment("app-a")
