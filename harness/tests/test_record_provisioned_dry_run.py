...
class RecordProvisionedDryRunTests(unittest.TestCase):
    ...
    def test_dry_run_still_enforces_the_resource_name_binding_check(self):
        # A dry run must not become a way to bypass the exact-match app_context binding check
        # (GH-29) -- the binding check must run BEFORE the dry_run branch, not be skipped by it.
        env_path = Path(self.tmpdir.name) / "environments.json"
        env_lock_path = Path(self.tmpdir.name) / ".environments.lock"
        with unittest.mock.patch.object(environments_store, "ENVIRONMENTS_PATH", env_path), \
             unittest.mock.patch.object(environments_store, "ENVIRONMENTS_LOCK_PATH", env_lock_path), \
             unittest.mock.patch("subprocess.run") as mock_run:
            created = mcp_server.create_environment("real-app")
            app_context = created["app_context"]
            result = mcp_server.record_provisioned(
                app_context, "file-storage", "other-app::photos", dry_run=True
            )
        mock_run.assert_not_called()
