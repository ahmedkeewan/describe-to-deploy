...
two processes racing a read-modify-write cycle on the same file (stack-state.json,
environments.json, environment-snapshots.json, events.jsonl) can silently
...
See research/agdr/AgDR-0002 for why flock was chosen over a database.
"""
import contextlib
import fcntl
...
def locked(lock_path: Path) -> Iterator[None]:
    ...
    it protects. Do not hold the lock across a slow operation such as a verify command --
    verification stays outside the locked section so one slow check never blocks
